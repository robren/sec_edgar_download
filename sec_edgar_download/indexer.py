...
def _parse_xbrlfiles(edgar_sub_elem, edgar_ns, item):
    ...
    xbrl_files = edgar_sub_elem.findall('.//edgar:' +'xbrlFile', namespaces=edgar_ns)
...
class SecIndexer():
    def __init__(self, work_dir="edgar/"):
        ...
        self.edgar_keys = ('company_name', 'form_type', 'filing_date', 'cik_number',
                           'accession_number', 'file_number', 'acceptance_datetime',
                           'period', 'assistant_director', 'assigned_sic',
                           'fiscal_year_end', 'xbrl_files'
                          )

        self.edgar_labels = ('companyName', 'formType', 'filingDate', 'cikNumber',
                             'accessionNumber', 'fileNumber', 'acceptanceDatetime',
                             'period', 'assistantDirector', 'assignedSic',
                             'fiscalYearEnd', 'xbrlFiles'
                            )
    ...
    def download_sec_feeds(self, from_year, to_year, from_month=1, to_month=12):
    ...
    def download_xbrl_data(self, cik, from_year, to_year, form_type='All'):
        ...
        for the period in question using the CLI to invoke download_sec_feeds().
        Downloads files to the subdirectory "filings" within the directory set by the
        work_dir class variable. This defaults to "./edgar" within the directory the
        ...
        logging.debug('download_xbrl_data: cik = %s, form_type = %s,' \
                'from_year = %d, to_year = %d', cik, form_type, from_year,\
                to_year)
        ...
        if form_type == 'All':
            mask = (df['filing_date'] >= from_date) & (df['filing_date'] <= to_date) &\
                    (df['cik_number'] == cik)

        else:
            mask = (df['filing_date'] >= from_date) & (df['filing_date'] <= to_date) &\
                    (df['cik_number'] == cik) & (df['form_type'] == form_type)
    ...
    def _download_sec_feed(self, year, month):
        ...
        Downloads RSS feeds from the SEC edgar website for a given year and month.
        The feeds are stored by year and month, each containing details of all of the
        filings made to the SEC for that month
        ...
        feed_filename = 'xbrlrss-' + str(year) + '-' + '{:02}'.format(month) + '.xml'
        logging.debug('feed_filename = %s', feed_filename)

        feed_file = os.path.join(self.feed_dir, feed_filename)

        if not os.path.exists(feed_file):
            edgar_filings_feed = 'http://www.sec.gov/Archives/edgar/monthly/' + feed_filename
            logging.debug('Edgar Filings Feed =%s', edgar_filings_feed)

            try:
                response = requests.get(edgar_filings_feed, timeout=1)
                response.raise_for_status()
            except requests.exceptions.Timeout as err:
                logging.warning("Timeout %s", err)
    ...
    def parse_sec_rss_feeds(self, rss_filename):
        ...
        the actual xbrl format filing itself. Each feedfile contains the details
        of a months worth of filings for all filers.
        ...
        for item in items:
            for key, label in zip(self.edgar_keys, self.edgar_labels):
                edgar_sub_elem = item.find('.//edgar:' + label, namespaces=edgar_ns)
    ...
    def _prep_database_table(self):
        ...
        To ensure that the  accesson_number is marked  as a PRIMARY KEY we need to
        manually create the db table. Pandas does not create a table with a
        PRIMARY KEY; without ths, repeated updates from subequent runs on the same
        feed would cause the database to get duplicate entries and grow!
        """
        columns = ','.join(self.edgar_keys)
        columns = re.sub('accession_number', 'accession_number PRIMARY KEY', columns)
        table_parms = '''CREATE TABLE IF NOT EXISTS feeds ({})'''.format(columns)
        print(table_parms)
    ...
    def _save_dicts_to_database(self, dicts):
        """
        Takes a list of dictionaries, converts to a pandas dataframe then stores this
